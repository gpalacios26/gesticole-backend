...
import com.gregpalacios.gesticole.util.EmailUtil;
import com.gregpalacios.gesticole.util.MailDTO;
...
public class LoginController {
	...
	public ResponseEntity<Integer> enviarCorreo(HttpServletRequest request) throws Exception {
		...
		if (user != null && user.getIdUsuario() > 0) {
			...
			mailDTO.setFrom("GestiCole");
			mailDTO.setTo(user.getUsername());
			mailDTO.setSubject("GestiCole - Restablecer Contraseña");
