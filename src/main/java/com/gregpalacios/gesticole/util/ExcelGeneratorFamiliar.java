...
public class ExcelGeneratorFamiliar {

	public static ByteArrayInputStream listadoToExcel(List<Familiar> familiares) throws IOException {
		String[] COLUMNs = { "ID", "Nombres", "Apellidos", "Tipo Documento", "Número Documento", "Fecha Nacimiento", 
				"Sexo", "Teléfono Celular", "Teléfono Fijo", "Correo", "Familiar del Alumno", "Parentesco" };
		try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
            ...
            cell_Title.setCellValue("LISTADO DE FAMILIARES DE LOS ALUMNOS");
			...
			// Content
			int rowIdx = 4;
			for (Familiar datos : familiares) {
				Row row = sheet.createRow(rowIdx++);

				Cell cell_1 = row.createCell(0);
				cell_1.setCellValue(datos.getIdFamiliar());
				...
				cell_11.setCellValue(datos.getAlumno().getNombres() + ' ' + datos.getAlumno().getApellidos());
				cell_11.setCellStyle(BodyCellStyle);
				
				Cell cell_12 = row.createCell(11);
				cell_12.setCellValue(datos.getParentesco().getDescripcion());
