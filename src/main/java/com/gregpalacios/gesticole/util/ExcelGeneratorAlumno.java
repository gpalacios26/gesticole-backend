...
public class ExcelGeneratorAlumno {

	public static ByteArrayInputStream listadoToExcel(List<Alumno> alumnos) throws IOException {
		String[] COLUMNs = { "ID", "Nombres", "Apellidos", "Tipo Documento", "Número Documento", "Fecha Nacimiento", 
				"Sexo", "Teléfono Celular", "Teléfono Fijo", "Correo" };
		try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
            ...
            cell_Title.setCellValue("LISTADO DE ALUMNOS");
			...
			// Content
			int rowIdx = 4;
			for (Alumno datos : alumnos) {
				Row row = sheet.createRow(rowIdx++);

				Cell cell_1 = row.createCell(0);
				cell_1.setCellValue(datos.getIdAlumno());
